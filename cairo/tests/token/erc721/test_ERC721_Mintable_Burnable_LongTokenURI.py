...
def contract_defs():
    ...
    erc721_def = get_contract_def(
        'openzeppelin/token/erc721/ERC721_Mintable_Burnable_LongTokenURI.cairo')
...
async def erc721_init(contract_defs):
    ...
    tokenURI_felt_array = str_to_felt_array("https://testnet.playoasis.xyz/longurl/0x123456/tokenuri/")
    erc721 = await starknet.deploy(
        ...
        constructor_calldata=[
            ...
            len(tokenURI_felt_array),           # baseTokenURI
            *tokenURI_felt_array,
        ]
    )
...
async def test_tokenURI(erc721_minted):
    ...
    decodedTokenURI = felt_array_to_str(execution_info.result.tokenURI)
    assert decodedTokenURI == "https://testnet.playoasis.xyz/longurl/0x123456/tokenuri/{}".format(from_uint(token_1))

    execution_info = await erc721.tokenURI(token_2).invoke()
    decodedTokenURI = felt_array_to_str(execution_info.result.tokenURI)
    assert decodedTokenURI == "https://testnet.playoasis.xyz/longurl/0x123456/tokenuri/{}".format(from_uint(token_2))


    # setTokenURI
    tokenURI_felt_array = str_to_felt_array("https://briq.construction/api/tokenid/")
    await signer.send_transaction(
        account, erc721.contract_address, 'setTokenURI', [
            len(tokenURI_felt_array),
            *tokenURI_felt_array
        ]
    )
    ...
    execution_info = await erc721.tokenURI(token_1).invoke()
    decodedTokenURI = felt_array_to_str(execution_info.result.tokenURI)
    assert decodedTokenURI == "https://briq.construction/api/tokenid/{}".format(from_uint(token_1))

    execution_info = await erc721.tokenURI(token_2).invoke()
    decodedTokenURI = felt_array_to_str(execution_info.result.tokenURI)
    assert decodedTokenURI == "https://briq.construction/api/tokenid/{}".format(from_uint(token_2))
...
async def test_setTokenURI_from_not_owner(erc721_minted):
    ...
    tokenURI_felt_array = str_to_felt_array("https://this.tokenuri.shouldnot.be.set/api/")
    await assert_revert(signer.send_transaction(
